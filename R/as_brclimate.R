...
as_brclimate <- function(x, varnames = NULL) {
  ...
  if (any(class(x) == "list")) {       
    ...
    ## loop through list again, and put everything in place in the new
    ## output matrix
    for (i in 1:n) {
      y <- x[[i]]
      for (j in 1:nyrs) {
    ...
    if (dim(x)[2] == 13) {              
      ## check if the first column is perfect sequence of integer
      ## years. if expression evaluates to FALSE, then this is the
      ## case, if TRUE: stop.
      ...
      ## check if the first column is a perfect sequence of integer
      ## years, each repeated 12 times. if expression evaluates to
      ## FALSE, then this is the case, else stop.
      if (!any(x[,1] == rep(x[1,1]:x[dim(x)[1],1], each = 12))) {
        stop("unknown format of climate data")
      } else {
        if (!(any(x[,2] == rep(1:12, length(unique(x[,1])))))) {
          ## check if the second column is perfect sequence of 1:12 as
          ## often as there are individual years in column 1. if
          ## expression evaluates to FALSE, then this is the case,
          ## else stop.
          stop("unknown format of climate data")
        } else {
