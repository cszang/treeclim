...
##'   list of several of the latter described \code{data.frame} or
##'   \code{matrices}. As an internal format dispatcher checks the
##'   format automatically, it is absolutely necessary that in all
##'   three cases, only complete years (months 1-12) are provided. It
##'   is not possible to mix different formats in one go.
##'   
##'   The `complete` parameter specifies the months of the current
##'   year in which tree-growth is assumed to finish. This month marks
##'   the last month of the first season, and starting from here, 14
##'   different seasons are computed for each specified season length
##'   in one-month steps. E.g., for a starting value of 9 (current
...
##'   when one of the variables is named "temp"). The correlation of
##'   the primary variable with tree-growth is computed as the simple
