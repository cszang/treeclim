## this is the workhorse function of bootres2, the moving case is also
## handled with this now (major interface change to bR 1.X)
...
##'   programme DENDROCLIM2002 (Biondi and Waikul, 2004), and will calculate 
##'   bootstrapped (and non-bootstrapped) moving and static response and 
##'   correlation functions in a similar fashion as described in the above 
##'   mentioned paper. Important extensions include a very flexible parameter 
##'   selection model (see below), the possibility to use an unlimited number of
##'   climate parameters, and the option to use exact bootstrapping.
##'   
...
##'   ways. The default value is -6:9. This is equivalent to the standard 
##'   settings in DENDROCLIM2002 and bootRes 1.X, and selects from all climate 
##'   variables all months from previous year's June (-6, previous year's months
##'   are specified as negative integers) to current years September (9, months 
##'   of the current year are specified as positive integers) as model 
##'   parameters.
##'   
...
##'   \code{.range} corresponds the example above, where all specified months 
##'   are used, while \code{.sum} and \code{.mean} will use the sums and means 
##'   of the specified months. These modifiers also allow to select specific 
##'   climatic variables, addressed by name. Thus, \code{.mean(4:8, "temp")} 
##'   will select the mean for climate parameter "temp" for the months April to 
##'   August. Not only ranges, but also individual vectors can be used for month
##'   specification, like e.g., \code{.range(c(1, 3, 4, 5)}.
##'   
##'   The modifiers can be chained together using the `+` symbol, which makes it
##'   possible to create arbitrarily complex selections of climate parameters 
##'   for calibration.  E.g., \code{.mean(2:5, "temp") + .sum(2:5, "prec")} will
...
##'   of lists that can be chained together, bootres2 will not check for 
##'   meaningful specifications. Testing smart hypotheses is up the researcher.
##'   
##'   For the exclusion of months, the convenience function \code{excludefrom()}
##'   (or short \code{exfr()}) is provided. E.g., 
##'   \code{.range(excludefrom(-6:10, -11:3))} will yield the monthly values of 
##'   all parameters for the months previous June (-6) to current October (10), 
##'   but without the months previous November (-11) to current March (3) in 
##'   between. While it is also possible to supply arbitrary vectors as month 
##'   specification, and not only ranges as shown in most of the examples here, 
##'   this way of excluding e.g. the dormant season is far more convenient.
##'   
...
##'   removed using the PVP criterion (Guiot, 1990), principal component scores 
##'   are then calculated from the matrices of reduced eigenvectors and 
...
##' @param climate either a \code{data.frame} or \code{matrix} with climatic 
##'   data in monthly resolution, with year, month and climate parameters in 
##'   columns (all columns except year and month will be recognized as 
##'   parameters for response or correlation function), or a single 
##'   \code{data.frame} or \code{matrix} in 13-column format (see below), or 
##'   list of several of the latter.
##' @param selection either a numeric vector, a modifier, or a chain of 
##'   modifiers specifying the parameter selection for the model (see Details).
##' @param method \code{character} string specifying the calculation method. 
##'   Possible values are \dQuote{response} and \dQuote{correlation}. Partial 
##'   strings are ok.
##' @param moving \code{logical}; should the analyis be carried out in moving 
##'   windows. Defaults to \code{FALSE}.
##' @param win_size integer giving the window size for each recalculation in 
##'   years.
##' @param win_offset integer giving the number of years between each window 
##'   start in years.
##' @param start_last \code{logical} flag indicating whether the first window 
##'   should start at the rear end (youngest part of the series) or not.
...
##' @param ci \code{numerical} value to set the test level for significance test
##'   (values 0.01, 0.05 and 0.1 are allowed); the confidence intervals are 
##'   adapted accordingly.
##' @param boot \code{character} indicating which bootstrap method should be
##'   used, one of \code{c("std", "exact")}
##' @param sb \code{logical} flag indicating whether textual status bar for 
                ...
                )
{
  ...
  ## check, if we have months in the correct numerical
  ## representation, and preevaluate month specification for correct
  ## data truncation
  ...
  ## generate design matrix for calibration by evaluating the
  ## selections
  design <- br_design(selection, climate_pmat)

  ## check if number of parameters is smaller than number of
  ## observations
  ...
  ## pass chrono and design matrix to the respective analysis
  ## functions
