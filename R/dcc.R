...
#' correlation functions from tree-ring chronologies and monthly climatic data. 
#' For the moving case, the calculation is performed repeatedly for consecutive 
#' time windows. Function parameters may be bootstrapped to calculate their 
...
#'   settings in DENDROCLIM2002 and bootRes, and selects from all climate 
...
#'   
#'   1000 bootstrap samples are taken from the original distributions of climate
#'   and tree-ring data, either using the stationary bootstrap (Politis and 
#'   Romano 1994, \code{boot = "stationary"}) or classical bootstrap 
#'   (DENDROCLIM2002-style, \code{boot = "std"}). The stationary bootstrap 
#'   mimics the stationary properties of the original time series in the 
#'   resampled time series by resampling within blocks. Within each block, the 
#'   number of observations is random and has a geometric distribution. 
#'   Consequently, the choice of the distribution parameter will affect the 
#'   autocorrelation structure of the resampled time series. Optimal (expected) 
#'   block length is chosen according to Politis and White (2004). In the case 
#'   of response function analysis, an eigen decomposition of the standardized 
...
#'   seascorr (Meko et al. 2011, \code{boot = "exact"}). In this case, circulant
#'   embedding is used to simulate the tree-ring data 1000 times as time series 
#'   with the same frequency characteristics like the original time-series 
#'   (Percival & Constantine, 2006). Empirical non-exceedence probabilities are 
#'   used to test the coefficients of the response/correlation function with the
#'   original data for significance. For the exact bootstrapping case, no 
#'   confidence intervals for the response/correlation coefficients can be 
#'   computed.
...
#'   strings are ok.
#' @param dynamic \code{character}; should the analyis be carried out in a 
#'   static, moving, or evolving fashion. One of \code{c("static", "moving", 
#'   "evolving")}. Defaults to \code{"static"}. Partial strings are ok.
#' @param moving deprecated, use \code{dynamic} instead.
#' @param win_size integer giving the window size for each recalculation in 
#'   years for a moving analysis, and the initial window size for an evolving 
#'   analysis.
...
#'   used, one of \code{c("stationary", "std", "exact")}
...
#'   
#'   The functions 'summary' and 'plot' are used to obtain and print a summary 
#'   of the results, and to create a plot. The function 'coef' can be used to 
#'   extract the coefficients.
#'   
#'   An object of class '"tc_dcc"' is a list containing at least the following 
#'   components:
#'   
#'   \item{call}{the call made to function 'dcc'}
#'   
#'   \item{coef}{the coefficients, themselves being an object of class 'tc_coef'
#'   for the static case, and of class 'tc_mcoef' for the moving case. Objects 
#'   of class 'tc_coef' are single data.frames, while objects of class 
#'   'tc_mcoef' are lists of seperate data.frames for the coefficients ($coef), 
#'   upper and lower confidence interval ($ci_upper and $ci_lower), and 
#'   significance flags ($significant)}
#'   
...
#'   
#'   Patton, A. and D.N. Politis and H. White (2009), "CORRECTION TO Automatic 
#'   block-length selection for the dependent bootstrap" by D. Politis and H. 
#'   White", Econometric Reviews 28(4), 372-375.
#'   
#'   Politis, D.N. and H. White (2004), Automatic block-length selection for the
