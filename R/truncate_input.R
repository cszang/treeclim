...
##' @param timespan the timespan for truncating as vector with min and
##' max year
...
truncate_input <- function(chrono, climate, timespan = NULL, minmonth,
  ...
  if (minmonth < 0 && is.na(match((start_year - 1), climate_years))) { 
    offset <- 1
    if (is.null(timespan)) {
      warning(paste("bootres2 tries to use the maximum overlap in timespan for chronology and climate data. The overlap starts in ",
                    start_year,
